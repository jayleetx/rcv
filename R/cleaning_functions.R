...
#'
#' Takes a data frame of a single column (i.e.
#' sf_bos_ballot) and splits it into usable columns.
#'
#' @param data A data frame containing the ballot image data
#' @param format Which RCV format the ballot image is in
#' @return A data frame with multiple columns
#' @examples
#' read_ballot(data = sf_bos_ballot, format = "California")
#' @importFrom dplyr %>%
...
#' @return A data frame with multiple columns
#' @examples
#' read_ballot(data = sf_bos_ballot)
#' @importFrom dplyr %>%
...
label_lookup <- function(data) {
  ...
  else if(tools::file_ext(data) == "csv" & grep(".TITLE", cambridge_lookup[1,1]) == 1) {
    data
    # hey Jay write this part next week when you hear back from Theo
  }
